...
- `npm run reddit-query -- "<query>"` - Print the same filtered Reddit results the per-post Reddit card would show for a given query. Cross-imports `frontend/lib/reddit.ts` so backend/sort/filter match the live API exactly. Use it to prototype a `reddit_query` override before setting it. The Arctic Shift fallback fans out across the subreddits in the Redis list `__doodles:reddit-subreddits` (defaults: Watches, MicrobrandWatches, VintageWatches, JapaneseWatches) — see README "Reddit Card".
